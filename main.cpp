...
// Forward declarations (defined later, used by persistence code in DB classes)
...
class HNSW {
    ...
    std::vector<std::pair<float,int>> searchLayer(
        ...
        std::vector<int>* visitOrder = nullptr)
    {
        ...
        cands.push({d0, ep});
        found.push({d0, ep});
        if (visitOrder) visitOrder->push_back(ep);
        ...
        while (!cands.empty()) {
            ...
            for (int nid : G[cid].nbrs[lyr]) {
                ...
                if ((int)found.size() < ef || nd < found.top().first) {
                    cands.push({nd, nid});
                    found.push({nd, nid});
                    if (visitOrder) visitOrder->push_back(nid);
...
public:
    ...
    // Search trace: records the greedy descent path through upper layers
    // and the order in which nodes are expanded during the layer-0 search.
    // Used for the "Search Trace Visualization" feature.
    struct Trace {
        std::vector<std::pair<int,int>> descent;   // {layer, entryNodeId}
        std::vector<int> expansion;                // layer-0 expansion order
        std::vector<std::pair<float,int>> result;  // final k results
        int topLayer;
    };

    Trace knnTrace(const std::vector<float>& q, int k, int ef, DistFn dist) {
        Trace tr; tr.topLayer = topLayer;
        if (entryPt == -1) return tr;
        int ep = entryPt;
        tr.descent.push_back({topLayer, ep});
        for (int lc = topLayer; lc > 0; lc--) {
            if (lc < (int)G[ep].nbrs.size()) {
                auto W = searchLayer(q, ep, 1, lc, dist);
                if (!W.empty()) ep = W[0].second;
            }
            tr.descent.push_back({lc - 1, ep});
        }
        std::vector<int> order;
        auto W = searchLayer(q, ep, std::max(ef, k), 0, dist, &order);
        if ((int)W.size() > k) W.resize(k);
        tr.expansion = order;
        tr.result = W;
        return tr;
...
public:
    ...
    struct SearchOut {
        std::vector<Hit> hits; long long us; std::string algo, metric;
        bool hasTrace = false;
        std::vector<std::pair<int,int>> traceDescent;  // {layer, nodeId}
        std::vector<int> traceExpansion;               // layer-0 visit order
    };
                     ...
                     const std::string& category = "")
    {
        ...
        // Over-fetch when filtering by category so we still return k results
        int fetchK = category.empty() ? k : std::min((int)store.size(), std::max(k * 6, k + 10));
        auto t0  = std::chrono::high_resolution_clock::now();

        std::vector<std::pair<float,int>> raw;
        SearchOut out; out.algo = algo; out.metric = metric;

        if (algo == "bruteforce") {
            raw = bf.knn(q, fetchK, dfn);
        } else if (algo == "kdtree") {
            raw = kdt.knn(q, fetchK, dfn);
        } else {
            HNSW::Trace tr = hnsw.knnTrace(q, fetchK, 50, dfn);
            raw = tr.result;
            out.hasTrace = true;
            out.traceDescent  = tr.descent;
            out.traceExpansion = tr.expansion;
        }
        ...
        for (auto& [d, id] : raw) {
            ...
            if (!category.empty() && store[id].category != category) continue;
            out.hits.push_back({id, store[id].metadata, store[id].category, store[id].emb, d});
            if ((int)out.hits.size() >= k) break;
        }
        return out;
    }

    struct BenchOut {
        double bfUs, kdUs, hnswUs;       // averaged microseconds
        double speedupKdVsBf, speedupHnswVsBf;
        int n, reps;
    };

    BenchOut benchmark(const std::vector<float>& q, int k, const std::string& metric, int reps = 30) {
        std::lock_guard<std::mutex> lk(mu);
        auto dfn  = getDistFn(metric);
        auto avgUs = [&](auto fn) -> double {
            long long total = 0;
            ...
            return (total / (double)reps) / 1000.0;  // ns -> us
        };
        double bfUs   = avgUs([&]{ bf.knn(q, k, dfn); });
        double kdUs   = avgUs([&]{ kdt.knn(q, k, dfn); });
        double hnswUs = avgUs([&]{ hnsw.knn(q, k, 50, dfn); });
        BenchOut out;
        out.bfUs = bfUs; out.kdUs = kdUs; out.hnswUs = hnswUs;
        out.speedupKdVsBf   = kdUs   > 0 ? bfUs / kdUs   : 0;
        out.speedupHnswVsBf = hnswUs > 0 ? bfUs / hnswUs : 0;
        out.n = (int)store.size();
        out.reps = reps;
        return out;
    }
    ...
    // ── PERSISTENCE ──────────────────────────────────────────────────
    // One JSON object per line (JSONL) under data/vectors.jsonl
    ...
    void loadFromFile(const std::string& path, DistFn dist) {
        std::ifstream f(path);
        if (!f.is_open()) return;
        std::string line;
        while (std::getline(f, line)) {
            if (line.empty()) continue;
            int id = extractInt(line, "id", -1);
            if (id < 0) continue;
            std::string meta = extractStr(line, "metadata");
            std::string cat  = extractStr(line, "category");
            ...
            store[id] = v; bf.insert(v); kdt.insert(v); hnsw.insert(v, dist);
...
public:
    ...
    void loadFromFile(const std::string& path) {
        ...
        while (std::getline(f, line)) {
            ...
            int id = extractInt(line, "id", -1);
            if (id < 0) continue;
            std::string title = extractStr(line, "title");
            std::string text  = extractStr(line, "text");
            size_t p = line.find("\"embedding\"");
            if (p == std::string::npos) continue;
            p = line.find('[', p);
            size_t e = line.find(']', p);
...
void loadDemo(VectorDB& db) {
    ...
    // 16D layout: OS[0-2] CN[3-5] DBMS[6-8] DSA[9-11] AI/ML[12-13] General/Fun[14-15]
    db.insert("Deadlock: circular wait, mutual exclusion, hold and wait, no preemption", "os",
        {0.85f,0.76f,0.80f,0.08f,0.11f,0.11f,0.12f,0.07f,0.09f,0.06f,0.08f,0.10f,0.06f,0.07f,0.11f,0.10f}, dist);
    db.insert("Semaphores and Mutex: synchronization primitives for mutual exclusion", "os",
        {0.79f,0.84f,0.87f,0.06f,0.12f,0.11f,0.08f,0.07f,0.13f,0.08f,0.07f,0.07f,0.12f,0.10f,0.12f,0.11f}, dist);
    db.insert("Paging and Virtual Memory: page tables, TLB, demand paging", "os",
        {0.84f,0.90f,0.81f,0.10f,0.12f,0.10f,0.12f,0.10f,0.11f,0.06f,0.08f,0.08f,0.07f,0.08f,0.07f,0.08f}, dist);
    db.insert("CPU Scheduling: FCFS, SJF, Round Robin, priority scheduling", "os",
        {0.85f,0.81f,0.81f,0.07f,0.08f,0.13f,0.11f,0.10f,0.07f,0.11f,0.07f,0.09f,0.13f,0.10f,0.10f,0.11f}, dist);
    db.insert("Process vs Thread: context switching and multithreading", "os",
        {0.88f,0.87f,0.79f,0.06f,0.08f,0.08f,0.07f,0.13f,0.12f,0.08f,0.11f,0.09f,0.12f,0.09f,0.08f,0.08f}, dist);
    db.insert("TCP vs UDP: connection-oriented vs connectionless transport protocols", "cn",
        {0.10f,0.08f,0.10f,0.89f,0.82f,0.79f,0.13f,0.10f,0.07f,0.06f,0.07f,0.10f,0.12f,0.09f,0.06f,0.09f}, dist);
    db.insert("HTTP and HTTPS: request-response model, TLS encryption", "cn",
        {0.13f,0.10f,0.13f,0.88f,0.76f,0.86f,0.11f,0.10f,0.08f,0.10f,0.07f,0.09f,0.09f,0.13f,0.12f,0.08f}, dist);
    db.insert("DNS and DHCP: domain name resolution and dynamic IP allocation", "cn",
        {0.10f,0.07f,0.12f,0.88f,0.80f,0.85f,0.10f,0.07f,0.11f,0.10f,0.11f,0.10f,0.06f,0.08f,0.06f,0.13f}, dist);
    db.insert("OSI Model: seven layers from physical to application", "cn",
        {0.12f,0.12f,0.08f,0.77f,0.88f,0.89f,0.07f,0.09f,0.06f,0.11f,0.11f,0.07f,0.09f,0.10f,0.08f,0.12f}, dist);
    db.insert("Subnetting and Routing Protocols: CIDR, OSPF, BGP", "cn",
        {0.09f,0.07f,0.10f,0.86f,0.79f,0.80f,0.13f,0.11f,0.09f,0.10f,0.07f,0.08f,0.08f,0.10f,0.08f,0.08f}, dist);
    db.insert("ACID Properties and Transactions: atomicity consistency isolation durability", "dbms",
        {0.06f,0.10f,0.08f,0.12f,0.12f,0.06f,0.79f,0.85f,0.79f,0.07f,0.13f,0.10f,0.09f,0.11f,0.12f,0.07f}, dist);
    db.insert("Normalization: 1NF, 2NF, 3NF, BCNF and removing redundancy", "dbms",
        {0.07f,0.09f,0.09f,0.09f,0.11f,0.11f,0.90f,0.77f,0.82f,0.08f,0.12f,0.08f,0.07f,0.09f,0.09f,0.08f}, dist);
    db.insert("Indexing and Joins: B-Tree indexes, inner and outer joins", "dbms",
        {0.08f,0.12f,0.09f,0.12f,0.10f,0.06f,0.90f,0.88f,0.90f,0.12f,0.12f,0.07f,0.09f,0.07f,0.09f,0.06f}, dist);
    db.insert("SQL Query Optimization: execution plans and query tuning", "dbms",
        {0.09f,0.13f,0.08f,0.11f,0.09f,0.09f,0.89f,0.90f,0.84f,0.11f,0.07f,0.08f,0.13f,0.10f,0.10f,0.11f}, dist);
    db.insert("Binary Search Tree: O(log n) search, insert, and delete", "dsa",
        {0.06f,0.10f,0.10f,0.12f,0.07f,0.13f,0.07f,0.07f,0.10f,0.85f,0.79f,0.78f,0.12f,0.08f,0.10f,0.10f}, dist);
    db.insert("AVL Tree: self-balancing BST with rotations", "dsa",
        {0.09f,0.10f,0.10f,0.13f,0.07f,0.11f,0.08f,0.09f,0.11f,0.80f,0.80f,0.87f,0.07f,0.09f,0.13f,0.13f}, dist);
    db.insert("Heap and Priority Queue: min-heap, max-heap, heapify", "dsa",
        {0.07f,0.07f,0.08f,0.13f,0.12f,0.12f,0.09f,0.07f,0.12f,0.86f,0.85f,0.90f,0.11f,0.06f,0.12f,0.08f}, dist);
    db.insert("Graph Traversal: BFS and DFS algorithms", "dsa",
        {0.11f,0.13f,0.07f,0.07f,0.07f,0.10f,0.08f,0.10f,0.11f,0.79f,0.85f,0.80f,0.09f,0.12f,0.12f,0.07f}, dist);
    db.insert("Dynamic Programming: memoization and overlapping subproblems", "dsa",
        {0.09f,0.08f,0.06f,0.11f,0.10f,0.08f,0.11f,0.10f,0.09f,0.76f,0.77f,0.88f,0.12f,0.10f,0.12f,0.10f}, dist);
    db.insert("Trie: prefix tree for fast string search", "dsa",
        {0.07f,0.07f,0.08f,0.12f,0.12f,0.12f,0.12f,0.07f,0.08f,0.77f,0.87f,0.88f,0.09f,0.10f,0.07f,0.13f}, dist);
    db.insert("Machine Learning: supervised, unsupervised, and reinforcement learning", "ai",
        {0.12f,0.13f,0.12f,0.12f,0.06f,0.11f,0.08f,0.13f,0.12f,0.12f,0.12f,0.08f,0.87f,0.78f,0.12f,0.12f}, dist);
    db.insert("Neural Networks and Deep Learning: backpropagation and gradient descent", "ai",
        {0.08f,0.12f,0.09f,0.08f,0.12f,0.08f,0.06f,0.07f,0.08f,0.12f,0.13f,0.08f,0.85f,0.82f,0.13f,0.10f}, dist);
    db.insert("Cloud Computing: IaaS, PaaS, SaaS and virtualization", "general",
        {0.13f,0.07f,0.13f,0.07f,0.13f,0.08f,0.07f,0.09f,0.11f,0.08f,0.10f,0.10f,0.09f,0.10f,0.80f,0.86f}, dist);
    db.insert("Cybersecurity: encryption, firewalls, and common attack vectors", "general",
        {0.06f,0.12f,0.10f,0.11f,0.11f,0.11f,0.09f,0.06f,0.11f,0.08f,0.08f,0.12f,0.11f,0.08f,0.80f,0.82f}, dist);
    db.insert("Football Offside Rule: explained simply for beginners", "fun",
        {0.09f,0.08f,0.07f,0.09f,0.13f,0.11f,0.12f,0.10f,0.08f,0.10f,0.06f,0.08f,0.09f,0.10f,0.85f,0.83f}, dist);
    db.insert("Why Pizza Became a Global Favorite Food", "fun",
        {0.09f,0.07f,0.09f,0.12f,0.12f,0.07f,0.07f,0.10f,0.10f,0.08f,0.12f,0.11f,0.11f,0.08f,0.79f,0.76f}, dist);
}
...
int main() {
    ...
    // Load persisted state, or fall back to demo data on first run
    db.loadFromFile("data/vectors.jsonl", getDistFn("cosine"));
    if (db.empty()) {
        loadDemo(db);
        db.persist("data/vectors.jsonl");
    }
    docDB.loadFromFile("data/docs.jsonl");
    ...
    svr.Get("/search", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        auto category = req.get_param_value("category"); // optional, e.g. "cs"

        auto out = db.search(q, k, metric, algo, category);
        ...
        // Search trace (HNSW only) — for the Search Trace Visualization feature
        if (out.hasTrace) {
            ss << ",\"trace\":{\"topLayer\":" << (out.traceDescent.empty() ? -1 : out.traceDescent[0].first)
               << ",\"descent\":[";
            for (size_t i = 0; i < out.traceDescent.size(); i++) {
                if (i) ss << ',';
                ss << "{\"layer\":" << out.traceDescent[i].first
                   << ",\"id\":"    << out.traceDescent[i].second << '}';
            }
            ss << "],\"expansion\":[";
            for (size_t i = 0; i < out.traceExpansion.size(); i++) {
                if (i) ss << ',';
                ss << out.traceExpansion[i];
            }
            ss << "]}";
        }
        ss << '}';
    ...
    svr.Delete(R"(/delete/(\d+))", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        if (ok) db.persist("data/vectors.jsonl");
    ...
    svr.Get("/benchmark", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        auto metric = req.get_param_value("metric"); if (metric.empty()) metric = "cosine";
        int reps = 30; try {
            auto rp = req.get_param_value("reps");
            if (!rp.empty()) reps = std::max(1, std::min(200, std::stoi(rp)));
        } catch (...) {}
        auto b = db.benchmark(q, k, metric, reps);
        std::ostringstream ss;
        ss << std::fixed << std::setprecision(2);
        ss << "{\"bruteforceUs\":" << b.bfUs
           << ",\"kdtreeUs\":"     << b.kdUs
           << ",\"hnswUs\":"       << b.hnswUs
           << ",\"speedupKdVsBruteforce\":"   << b.speedupKdVsBf
           << ",\"speedupHnswVsBruteforce\":" << b.speedupHnswVsBf
           << ",\"itemCount\":" << b.n
           << ",\"reps\":"      << b.reps << '}';
        res.set_content(ss.str(), "application/json");
    });
    ...
    svr.Delete(R"(/doc/delete/(\d+))", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        if (ok) docDB.persist("data/docs.jsonl");
        res.set_content("{\"ok\":" + std::string(ok ? "true" : "false") + "}",
                        "application/json");
    });
